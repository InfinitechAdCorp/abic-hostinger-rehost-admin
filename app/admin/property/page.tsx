...
export default function Property() {
  ...
  const { data, error, mutate } = useSWR<{
    ...
    message: string;
    records: Property[];
  }>(`${process.env.NEXT_PUBLIC_BASE_URL}/api/properties`, fetchWithToken);
      ...
      renderCell: (data: any) => (
        <div className="flex gap-2">
          <Link href={`/admin/property/${data.id}`} className="w-full">
            <Button
              size="sm"
              className="w-full uppercase font-semibold bg-yellow-300 text-yellow-800"
            >
              Details
            </Button>
          </Link>
      ...
      <div className="flex justify-between">
        ...
        <div className="flex justify-end items-center mb-4">
          <Button
            ...
            onPress={() => router.push("/admin/property/new-property")}
          >
            Add Property
          </Button>
        </div>
